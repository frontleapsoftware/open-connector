...
From a checkout or git dependency:
...
| Concern                                       | Inject                                                                                    |
| --------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Credentials, OAuth, tokens, runs, idempotency | `RuntimeDatabase` — typically `createNodeRuntimeDatabase({ backend: "postgresql", ... })` |
...
Apply Postgres migrations before opening the database (`npm run runtime:migrate` or the `migrate`
CLI entry). See [configuration.md](configuration.md).
...
`@mastra/core/workspace`, or a Local/GCS/S3 workspace FS). It does **not** depend on Mastra; pass
the filesystem instance you already construct for agents.
...
const runtimeDatabase = await createNodeRuntimeDatabase({
  backend: "postgresql",
  connectionString: process.env.OOMOL_CONNECT_DATABASE_URL!,
  secretCodec,
