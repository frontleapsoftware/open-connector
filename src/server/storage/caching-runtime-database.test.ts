...
describe("CachingRuntimeDatabase", () => {
  ...
  it("passes oauth state, run logs, idempotency, and marketplace through without caching", async () => {
    const inner = new FakeRuntimeDatabase();
    const database = new CachingRuntimeDatabase(inner, { bus: new MemoryPubSubBus() });
    ...
    expect(database.marketplaceStore).toBe(inner.marketplaceStore);
...
class FakeRuntimeDatabase implements RuntimeDatabase {
  ...
  readonly marketplaceStore = new FakeMarketplaceStore();
}
...
class FakeOAuthStateStore implements IOAuthStateStore {
  ...
  async deleteCreatedBefore(cutoffIso: string): Promise<void> {
    const cutoff = Date.parse(cutoffIso);
    for (const [key, value] of this.rows) {
      if (Date.parse(value.createdAt) < cutoff) {
...
class FakeRuntimeTokenStore implements IRuntimeTokenStore {
  ...
  async updatePolicy(id: string, policy: TokenPolicy): Promise<RuntimeTokenRecord | undefined> {
    ...
    const next = {
      ...existing,
      ...policy,
      allowedConnections: policy.allowedConnections ?? existing.allowedConnections,
    };
...
class FakeMarketplaceStore implements IMarketplaceStore {
  async getConfig(): Promise<StoredMarketplaceConfig | undefined> {
    return undefined;
  }

  async setConfig(_config: StoredMarketplaceConfig): Promise<void> {}
  ...
  async setProviderPreference(_preference: ProviderPreference): Promise<void> {}
}
